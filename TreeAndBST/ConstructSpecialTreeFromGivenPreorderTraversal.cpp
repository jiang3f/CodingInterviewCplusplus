...
namespace ConstructSpecialTreeFromGivenPreorderTraversal
{
	// Time Complexity : O(n)
	struct Node
	{
	...
	struct Node * BuildTree(struct Node * &rightMost, int pre[], char preLN[], int &pos)
	{
		bool isLeave = (preLN[pos] == 'L') ? true : false;

		struct Node * newNode = new struct Node(pre[pos++]);
		...
		newNode->left = BuildTree(rightMost, pre, preLN, pos);

		newNode->right = BuildTree(rightMost, pre, preLN, pos);

		return newNode;
	}

	int Run( int pre[], char preLN [], int sz)
	{
		struct Node *list = NULL;

		int pos = 0;

		BuildTree(list, pre, preLN, pos);

		return 0;
	}
};

namespace ConstructSpecialTreeFromGivenPreorderTraversal_Example
{
...
int ConstructSpecialTreeFromGivenPreorderTraversal_Test()
{
	...
	ConstructSpecialTreeFromGivenPreorderTraversal::Run(pre, preLN, sz);

	ConstructSpecialTreeFromGivenPreorderTraversal_Example::Run();
